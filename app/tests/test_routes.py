...
def test_get_answers_yes():
    ...
    response = client.get(url)
    assert "Merci d'avoir participé à ce sondage. Resultats: reponses:1 oui:1 non:0" in response.get_data(
        as_text=True)
    assert response.status_code == 200
...
def test_get_answers_no():
    ...
    response = client.get(url)
    assert "Merci d'avoir participé à ce sondage. Resultats: reponses:2 oui:1 non:1" in response.get_data(
        as_text=True)
    assert response.status_code == 200
